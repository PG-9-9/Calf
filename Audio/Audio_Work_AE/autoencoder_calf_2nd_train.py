...
def extract_features(audio, sample_rate, feature_extraction_logger,scaler_creater,output_dir):
    ...
    if not scaler_creater:
        scaler_path = os.path.join(output_dir, "2nd_scaler", "scaler.gz")
...
def fit_scaler_to_training_data(training_paths, sample_rate, evaluation_directory):
    ...
    scaler_file_path = os.path.join(evaluation_directory, '2nd_scaler','scaler.gz')
    try:
...
def test_consecutive_anomaly_per_window(model, feature_dir, threshold, min_consecutive_anomalies):
    anomaly_results = []
    for npz_file in sorted(os.listdir(feature_dir)):
