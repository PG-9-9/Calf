...
def extract_features(audio_windows, sample_rate):
    ...
    for window in audio_windows:
        ...
        spectral_centroids, spectral_rolloff, spectral_contrast = extract_spectral_features(window, sample_rate)
        zero_crossing_rate, autocorrelation = extract_temporal_features(window)
        chroma_stft, spec_bw, spec_flatness, rolloff, rms = extract_additional_features(window, sample_rate)
        all_features = np.hstack([mfccs, spectral_centroids, spectral_rolloff, spectral_contrast, zero_crossing_rate, autocorrelation, chroma_stft, spec_bw, spec_flatness, rolloff, rms])
...
def hyperparameter_tuning(root_path, evaluation_path, data_path, config_list, use_lstm=True):
    ...
    for config in config_list:
        ...
        audio_windows, filenames = load_and_window_audio_files_with_tracking(data_path['normal'], window_size, step_size, SAMPLE_RATE)
        ...
        if use_lstm:
            ...
            autoencoder = simplified_autoencoder(X_train_reshaped.shape[1], X_train_reshaped.shape[1])
        ...
        model_evaluation(autoencoder, X_val_reshaped, y_val, evaluation_path, model_type_label, X_train_reshaped) 

def train_model(X_train, X_val):
    logging.info(f"Model training with data shape: {X_train.shape}")
    try:
        autoencoder = simplified_autoencoder_with_lstm(X_train.shape[1], X_train.shape[2], 32)  
        autoencoder.fit(X_train, X_train, epochs=50, batch_size=32, validation_data=(X_val, X_val), callbacks=[EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)], verbose=1)
