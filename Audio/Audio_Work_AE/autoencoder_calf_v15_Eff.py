...
def load_and_window_audio_files(path, label, window_size, step_size, sample_rate, file_limit=26500):
    audio_windows = []
    labels = []
    file_map = {}  # Mapping of window index to file name
    file_count = 0
    window_index = 0  # Keep track of the global window index
    ...
    for filename in os.listdir(path):
        if filename.endswith('.wav'):
            ...
            for _ in windows:
                file_map[window_index] = filename
                window_index += 1
            file_count += 1
    return audio_windows, labels, file_map
...
def model_evaluation(autoencoder, X_test, y_test, evaluation_directory, model_type, X_abnormal,abnormal_file_map): #model, X_val, y_test, ev_path, label,X-abnormal, file_map
    ...
    # For Aggregated window mapping
    val_predictions = autoencoder.predict(X_test)
    abnormal_predictions = autoencoder.predict(X_abnormal)
    
    val_mse = np.mean(np.power(X_test - val_predictions, 2), axis=1)
...
def hyperparameter_tuning(root_path, evaluation_path, data_path, config_list, use_lstm=True):
    ...
    for config in config_list:
        ...
        abnormal_audio_windows, abnormal_labels,abnormal_file_map = load_and_window_audio_files(data_path['abnormal'], 1, window_size, step_size, SAMPLE_RATE)
        ...
        model_evaluation(autoencoder, X_val_reshaped, y_val, evaluation_path, model_type_label, X_abnormal_reshaped, abnormal_file_map)
