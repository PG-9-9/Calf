...
def concatenate_audio_files(file_paths, sample_rate):
    audio_sequence = []
    for file_path in file_paths:
        audio, _ = librosa.load(file_path, sr=sample_rate)
        audio_sequence.extend(audio)
    return np.array(audio_sequence)

def sliding_window_over_clips(audio_paths, window_size, step_size, sample_rate, num_clips):
    windows = []
    for i in range(0, len(audio_paths) - num_clips + 1, step_size):
        concatenated_audio = concatenate_audio_files(audio_paths[i:i + num_clips], sample_rate)
        num_samples_per_window = int(window_size * sample_rate)
        step_samples = int(step_size * sample_rate)
        for start in range(0, len(concatenated_audio) - num_samples_per_window + 1, step_samples):
            window = concatenated_audio[start:start + num_samples_per_window]
            windows.append(window)
    return windows
...
def load_and_window_audio_files(path, label, window_size, step_size, sample_rate, num_clips, file_limit=26500):
    audio_windows = []
    labels = []
    file_count = 0
    audio_paths = [os.path.join(path, f) for f in os.listdir(path) if f.endswith('.wav')][:file_limit]
    windows = sliding_window_over_clips(audio_paths, window_size, step_size, sample_rate, num_clips)
...
def hyperparameter_tuning(root_path, data_path, config_list):
    for config in config_list:
        ...
        normal_audio_windows, _ = load_and_window_audio_files(data_path['normal'], label=0, window_size=window_size, step_size=step_size, sample_rate=sample_rate,file_limit=2500,num_clips=3)
        abnormal_audio_windows, _ = load_and_window_audio_files(data_path['abnormal'], label=1, window_size=window_size, step_size=step_size, sample_rate=sample_rate,file_limit=34,num_clips=3)
...
if __name__ == "__main__":
    try:
        data_path = {
            'normal': "Calf_Detection/Audio/Audio_Work_AE/normal_calf_superset",
